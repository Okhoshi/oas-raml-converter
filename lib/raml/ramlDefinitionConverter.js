...
var RamlDefinitionConverter = function (_Converter) {
	...
	_createClass(RamlDefinitionConverter, [{
		...
		value: function _export(model) {
			...
			var attrIdSkip = ['name', 'type', 'reference', 'properties', 'items', 'compositionType', 'in', 'schema', 'additionalProperties', 'title', 'items', 'itemsList', 'exclusiveMaximum', 'exclusiveMinimum', 'readOnly', 'externalDocs', '$schema', 'annotations', 'collectionFormat', 'allowEmptyValue', 'fileReference', '_enum', 'error', 'warning', 'includePath', 'expanded'];
			...
			if (ramlDef.hasOwnProperty('properties')) {
				...
				for (var id in ramlDef.properties) {
					...
					if (id.startsWith('/') && id.endsWith('/')) {
						...
						if (!required.includes(id) && (!ignoreRequired && !isRaml08Version && !_value4.hasOwnProperty('required') || _value4.hasOwnProperty('required') && _value4.required === true)) required.push(id);

						if (_.isBoolean(_value4.required)) delete _value4.required;
